...
export const createPenyalur = async (
    ...
    try {
        const { nama_toko, kategori, nomor_whatsapp, alamat, latitude, longitude, user_id } = req.body;
        const newPenyalur = await db
            .insert(penyalurTable)
            .values({ nama_toko, kategori, nomor_whatsapp, alamat, latitude, longitude, user_id })
            .returning();
...
export const updatePenyalur = async (
  ...
  next: NextFunction,
    ...
    try {
        ...
        const { nama_toko, kategori, nomor_whatsapp, alamat, latitude, longitude } = req.body;
        const data: any = {};
